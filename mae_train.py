...
if __name__ == "__main__":
    ...
    # Batch Size
    parser.add_argument("-b", "--batch-size", dest = "batch_size", type = int, help = "Batch size per iteration", default = 8)
    # Dataset
    parser.add_argument(
        "-d",
        "--dataset",
    ...
    train_dataset = MVImgNetNeRF(dataset_path, split = 'train', number_of_imgs_from_the_same_scene = 4, patch_hw = [16,10], return_as_patches = False)
    validation_dataset = MVImgNetNeRF(dataset_path, split = 'test', number_of_imgs_from_the_same_scene = 4, patch_hw = [16,10], return_as_patches = False)
    ...
    lighting_module = SceneMaskAutoEncoder(epochs = epochs, model_parameters = model_parameters)
